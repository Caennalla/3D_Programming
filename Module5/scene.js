...
$(function(){
    ...
    var directionalLight = new THREE.DirectionalLight( 0xFAFA13, 1.0 ); 
    directionalLight.position.set( 1, 1, -1 ); 

    scene.add( directionalLight );

    //Add ambient light, simulating surround scattering light
    ambientLight = new THREE.AmbientLight(0x4C5BB9);
    scene.add( ambientLight  );
    

    scene.fog = new THREE.Fog(0x172747, 1.0, 50.0);
    // Add our flashlight, as no target is set it will point to 0,0,0
    var distance  = 6.0;
    var intensity = 2.0;
    spotLight = new THREE.SpotLight( 0x10F737, 
				     intensity,
				     distance ); 
	...
	//Version 1, not using lights:true, so no merging required
	//No idea how to use the spotlight values passed in the fragment shader
    ...
    var customPhongShader = new THREE.ShaderMaterial({
		...
		uniforms: {
				...
				// Position data for direct light
				"dirlight.pos": {
					type: 'v3',
					value: directionalLight.position
				},
				// Color data for direct light
				"dirlight.color": {
					type: 'v4',
					value: colorToVec4(directionalLight.color)
					
				},
				// Color data for ambient light
				u_ambient: {
					type: 'v4',
